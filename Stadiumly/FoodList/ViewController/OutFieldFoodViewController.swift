...
extension OutFieldFoodViewController: FoodSearchDelegate, KakaoMapEventDelegate {
    ...
    func poiDidTapped(kakaoMap: KakaoMap, layerID: String, poiID: String, position: MapPoint) {
        ...
        guard let layer = labelManager.getLabelLayer(layerID: layerID),
              let poi = layer.getPoi(poiID: poiID),
              let userObj = poi.userObject as? [String: Any],
              let url = userObj["url"] as? String
        else {
            print("❌ poiID '\(poiID)'에 해당하는 POI를 찾을 수 없거나 url이 없습니다.")
            return
        }
    ...
    func createPois() {
        ...
        for (index, place) in places.enumerated() {
            ...
            if let poi = layer.addPoi(option: poiOption,at: MapPoint(longitude: longitude, latitude: latitude)) {
                ...
                //POI 지도에 표시하기
                poi.show()
//                poiList.append(place.place_url)
                let poiID = "PoiLayer\(index)"
                poi.userObject = ["url": place.place_url, "customID": poiID] as AnyObject
